...
from src.data.interactions import build_positive_interaction_matrix
from src.evaluation.metrics import dcg, intra_list_diversity
from src.serving.cold_start import ColdStartPolicy
...
def test_model_config_records_retrieval_contract() -> None:
    ...
    assert config.get("model_name", config.get("version"))
...
def test_recommendation_metadata_is_client_friendly() -> None:
    ...
    recommender.recommend_detailed = lambda *_args, **_kwargs: {
        "items": [
            {
                "item_id": 1,
...
def test_cold_start_recommendation() -> None:
    ...
    recommender = Recommender.__new__(Recommender)
    recommender.cold_start_policy = ColdStartPolicy(
        popular_items=[10, 20, 30],
        genre_map={10: {"Action"}, 20: {"Comedy"}, 30: {"Action", "Sci-Fi"}},
        title_map={10: "Action Movie", 20: "Comedy Movie", 30: "Sci-Fi Movie"},
        popularity_counts={10: 500, 20: 300, 30: 200},
        log_popularity={10: 1.0, 20: 0.5, 30: 0.3},
    )

    _ids, _strategy, results = recommender.cold_start_recommend(
        preferred_genres=["Sci-Fi"], k=2
    )
...
def test_api_recommend_endpoint() -> None:
    ...
    client = TestClient(app)
    response = client.get("/recommend/1?k=5&debug=true")

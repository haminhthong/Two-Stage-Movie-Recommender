...
def intra_list_diversity(items: list[int], genres_map: dict[int, set[str]]) -> float:
    ...
    Được đo bằng khoảng cách Jaccard trung bình giữa tất cả các cặp item trong top-K list.
...
def evaluate_recommender(
    ...
    """Đánh giá toàn diện hệ thống gợi ý trên tập Test (Time-based split).
    ...
    # Ground truth từ tập Test: {user_id: true_item_id}
    ...
    for user_id in eligible_users:
        ...
        # Baseline phổ biến toàn cục, đồng thời loại item user đã xem trong train.
    ...
    metrics_summary = {
        ...
        # Tính trên tổng số lượt hiển thị (impressions) để đo Popularity Bias chính xác
...
def main() -> None:
    """Hàm main thực thi script đánh giá khi gọi từ CLI."""
